...
describe('TimePickerMenu', () => {
  ...
  it('renders 48 half-hour options', () => {
    const component = shallowMount(TimePickerMenu)
    expect(component.findAll('anonymous-stub')).toHaveLength(48)
  })

  it('renders options from 00:00 to 23:30', () => {
    const component = shallowMount(TimePickerMenu)
    const options = component.findAll('anonymous-stub')
  ...
  it('emits time-selected with the clicked value', async () => {
    const component = shallowMount(TimePickerMenu)
    await component.findAll('anonymous-stub').at(5).vm.$emit('click')
  ...
  it('highlights the option matching the value prop on mount', async () => {
    ...
    const highlighted = component.findAll('anonymous-stub').filter(w => w.attributes('data-highlight') === 'true')
  ...
  it('highlights the hovered option on mousemove', async () => {
    ...
    await component.findAll('anonymous-stub').at(10).trigger('mousemove')
    const highlighted = component.findAll('anonymous-stub').at(10)
