describe('KitInlineEdit--StringLine', () => {
    ...
    it('should focus input on click', () => {
        ...
        cy.get('[data-cy=editable] [data-cy=input-wrapper]').should(
            'have.css',
            'border-color',
            'rgb(76, 154, 255)'
        );
    ...
    it('should focus renderer on tab key', () => {
        ...
        cy.get('[data-cy=editable] [data-cy=label]').should(
            'have.css',
            'border-color',
            'rgb(76, 154, 255)'
        );
    ...
    it('should save edited value with enter', () => {
        cy.get('[data-cy=editable]').click();
        const input = cy.get(
            '[data-cy=editable] [data-cy=input-wrapper] input'
        );
        input.clear();
        input.type('Sample text');
        cy.get('body').type('{enter}');
        cy.get('[data-cy=editable] [data-cy=view-content]').contains(
            'Sample text'
        );
    });
    ...
    it('should cancel editing without save on click outside', () => {
        ...
        input.invoke('val').then((value) => {
            cy.get('[data-cy=editable] [data-cy=input-wrapper] input').type(
                'Sample text'
            );
            cy.get('body').click(0, 0);
            cy.get('[data-cy=editable] [data-cy=view-content]').contains(value);
        });
    });
    ...
    it('should stay focused when invalid', () => {
        ...
        const input = cy.get(
            '[data-cy=editable-error] [data-cy=input-wrapper] input'
        );
        input.clear();
        input.type('Sample text');
        cy.get('body').type('{enter}');
        cy.get('[data-cy=editable-error] [data-cy=input-wrapper] input').should(
            'have.focus'
        );
        cy.get('[data-cy=editable-error] [data-cy=input-wrapper]').should(
            'have.css',
            'border-color',
            'rgb(222, 53, 11)'
        );
