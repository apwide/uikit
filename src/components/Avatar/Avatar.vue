<template>
    <div class="outer" :style="`zIndex: ${zIndex}`">
        ...
        <component
            :is="tag" :href="link" target="_blank"
            class="wrapper" :square="`${square}`" :size="size"
            :style="style">
            <img
                v-if="avatar && !error" draggable="false" :src="avatar"
                alt="avatar" @error="error = true">
            <svg
                v-else viewBox="0 0 128 128" version="1.1"
            ...
            <component
                :is="status" v-if="status" class="status"
                :size="size"/>
            <component
                :is="presence" v-if="presence" :size="size"
