    ...
    <template #editor="{ props }">
      <KitSelect
        ...
        <template #option="{ option }">
          <div class="label">
            <KitUserRenderer :tag="tag" :user="option" :avatar-only="avatarOnly" />
          </div>
        </template>
        <template #selected="{ selected }">
          <div class="label">
            <KitUserRenderer :tag="tag" :user="selected" :avatar-only="avatarOnly" />
          </div>
        </template>
      </KitSelect>
    </template>
    <slot>
      <KitUserRenderer :tag="tag" :user="user" :avatar-only="avatarOnly" />
    </slot>
  </KitInlineEdit>
  <KitUserRenderer v-else :tag="tag" :user="user" :avatar-only="avatarOnly" />
</template>
